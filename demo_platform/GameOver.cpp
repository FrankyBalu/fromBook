...
const std::string MM::GameOverScene::s_ID = "GameOver";

void MM::GameOverScene::Update(){
...
void MM::GameOverScene::Render(){
    GuiLabel(m_LableGameOver, "Game Over");

    if (m_ActivButton == 1)
        GuiSetState(STATE_FOCUSED);
    else
        GuiSetState(STATE_NORMAL);
    GuiButton(m_ButtonNewGame, "Neues Spiel");
    GuiSetState(STATE_NORMAL);
    if (m_ActivButton == 2)
        GuiSetState(STATE_FOCUSED);
        else
...
bool MM::GameOverScene::OnEnter(){
    ...
    m_LableGameOver.x = m_ButtonNewGame.x = m_ButtonMainMenu.x = ButtonWidth;
    m_LableGameOver.width = m_ButtonNewGame.width = m_ButtonMainMenu.width =
ButtonWidth;
    m_LableGameOver.height = m_ButtonNewGame.height = m_ButtonMainMenu.height =
ButtonHeight;

    m_LableGameOver.y = ButtonHeight;
    m_ButtonNewGame.y = ButtonHeight * 3;
